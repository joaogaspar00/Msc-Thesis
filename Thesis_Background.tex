...
Note that, from here on, when a variable is written in any reference frame, it will be denoted by its correspondent superscript, e.g., if a variable is written in the rotor frame, it will be denoted by $x^r$, where $x$ is any variable.
...
where $\left[\mathbf{I}\right]$, in \unit{kg/m^2}, is the mass inertia tensor. 

To simply the approch of the vehicle dynamics, the rotor rotation along the $z$-axis is not considered in Eq. \ref{eq:momentum_equation} and another equation for the rotor's angular motion over its shaft is introduced further in Sec. \ref{section:rotor_dynamics}. Here one can look at the rotor as a \gls{cd} which has its center (hole) in the referece frame origin and the rotor referece frame, $O^r$, follows its rotation with $x$-axis and $y$-axis in plane with the \gls{cd}
...
For the rotor's dynamics, further considerations shall be made to properly account for the generation of forces acting on the payload's point-mass model and the moments that provide rotational freedom to the rotor's \gls{cd} model. In reality, the rotor rotates along its shaft, and this motion is crucial for its aerodynamic analysis. The rotational behavior directly influences the aerodynamic loads, which must be accurately modeled to capture the effects on both the rotor and the payload.

One of the primary concerns is the influence of rotational inertia and the torques that develop due to aerodynamic and inertial effects. The Eq. \ref{eq:rotation_eq} governing the rotor’s rotational dynamics can be expressed as
...
where $\Omega$ represents the angular acceleration, in \unit{rad/s}, and $\tau_z$ represents the angular momentum, computed as 
\begin{equation}
    \Omega = \frac{T}{I_{zz}},
\end{equation}

 with, $T$ is the applied torque, in \unit{N/m}, and $I_{zz}$ is the moment of inertia about the rotor’s principal axis of rotation. in \unit{kg/m^2}.

The goal of this approach is to effectively handle the high rotational velocities characteristic of rotor operation. When the rotor is spinning at high speeds, numerical stability becomes a concern, particularly when integrating the equations of motion. The time-step used in the \gls{ode} solver must be carefully selected to maintain accuracy without introducing excessive computational cost. A finer discretization may be required to capture the rapid variations in forces and moments while ensuring the solver remains efficient.

So instead of define a more finer discretization it is considered not only an blade load distribution, but also an azimultal load distribution for each time step which will not affect numerical stability or create difficults when the Eqs. \ref{eq:momentum_equation} and \ref{eq:rotation_eq} are coupled.
